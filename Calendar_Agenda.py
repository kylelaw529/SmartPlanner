...
class Agenda(Calendar):
    ...
    def __init__(self, tl, master=None, **kw):
        Calendar.__init__(self, master, normalbackground=colors['center_color'], weekendbackground=colors['center_color'],background=colors['top_color'], headersbackground=colors['top_color'], othermonthwebackground='#ededed',**kw)

        self.task_events = []
        self.tl = tl

        for task in self.tl.tasks:
            self.task_events.append((datetime.datetime.strptime(task.due_date, "%m/%d/%y"), task.name, self.tl.colors_dic[task]))
    ...
    def update(self):
        ...
        for task in self.tl.tasks:
            self.task_events.append(( datetime.datetime.strptime(task.due_date, "%m/%d/%y"), task.parent_course.name + " - " +task.name, self.tl.colors_dic[task]))

    def display_events(self):
        for event in self.task_events:
            self.calevent_create(event[0], event[1], event[2])
            self.tag_config(event[2], background=colors['center_color'], foreground="blue")
